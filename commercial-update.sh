...
if [ -f $SYSTEMD_PATH/$SERVICE_NAME ]; then 
        ...
        if ! cat $SYSTEMD_PATH/$SERVICE_NAME | grep "After="; then sed -i '/^Description/a\After=network-online.target' $SYSTEMD_PATH/$SERVICE_NAME; fi >/dev/null 2>&1
        if ! cat $SYSTEMD_PATH/$SERVICE_NAME | grep "Type=simple"; then sed -i '/^Service/a\Type=simple' $SYSTEMD_PATH/$SERVICE_NAME; fi >/dev/null 2>&1
